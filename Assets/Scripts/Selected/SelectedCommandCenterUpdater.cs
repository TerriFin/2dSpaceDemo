...
public class SelectedCommandCenterUpdater : MonoBehaviour, DescriptorUpdater {
    ...
    public Button selectedBuildingButton;
    ...
    public void CheckAndSetFactionChanges(string faction) {
        if (!FactionsManager.playerFaction.factionTag.Equals(faction)) {
            ...
            selectedBuildingButton.enabled = false;
            selectedBuildingButton.image.enabled = false;
        } else {
            ...
            selectedBuildingButton.onClick.AddListener(HandleBuildingButtonClick);
        }
    }
    ...
    private void HandleBuildingButtonClick() {
        SelectionManager.SetDescription(Instantiate(buildingWindow));
    }
}
